...
class PdfSettingsObject extends ConverterSettingsObject_1.default {
    ...
    constructor(clone) {
        ...
        if (clone) {
            if (clone.newPageOnSection !== undefined)
                this.newPageOnSection = clone.newPageOnSection;
            if (clone.contentZoom !== undefined)
                this.contentZoom = clone.contentZoom;
            if (clone.customHeader !== undefined)
                this.customHeader = clone.customHeader;
            if (clone.customFooter !== undefined)
                this.customFooter = clone.customFooter;
            if (clone.headerHeight !== undefined)
                this.headerHeight = clone.headerHeight;
            if (clone.footerHeight !== undefined)
                this.footerHeight = clone.footerHeight;
            if (clone.customStyleFile !== undefined)
                this.customStyleFile = clone.customStyleFile;
        }
    }
}
