...
var escapeSectionName = function (name) {
    // there should be no single \, since they have to be escaped in markdown
    return name.replace(/\|/g, "\\|");
};
var unescapeSectionName = function (name) {
    // unescape \\ needs to be done everywhere when interpreting markdown code
    return name.replace(/\\\|/g, "|").replace(/\\\\/g, "\\");
};
var escapeHTMLQuotes = function (text) {
    return text.replace(/"/g, "&quot;");
};
var removeMarkdownSyntax = function (text, converter) {
    return converter.makeHtml(text).replace(/<.*?>/g, "");
};
var descrIndex = 0;
var descriptions = {};
var addSectionOnHeading = {
    type: 'lang',
    filter: function (text, converter) {
        var headingDepth = converter.getOption('headingDepth');
        var useSubSections = converter.getOption('useSubSections');
        var subSection = converter.getOption('subSectionLevel');
        var subSubSection = converter.getOption('subsubSectionLevel');
        // parse headings
        if (converter.getOption('newSectionOnHeading')) {
            var match = text.match(inlineHeadingRegExp);
            if (match && match.length) {
                text = text.replace(inlineHeadingRegExp, function (wholeMatch, type, content) {
...
var replaceSectionSyntax = {
    ...
    filter: function (text, converter) {
        ...
        if (match && match.length) {
            // replace only first
            text = text.replace(firstSectionRegExp, function (wholeMatch, wrap, heading, addition) {
                return parseSection(conv, wholeMatch, wrap, heading, addition);
            });
            // replace all following
            text = text.replace(sectionRegExp, function (wholeMatch, wrap, heading, addition) {
...
var insertSectionDescription = {
    type: 'output',
    filter: function (text, converter) {
        text = text.replace(secDescriptionReplacementRegExp, function (wholeMatch, index) {
    ...
    filter: function (text, converter) {
        ...
        text.replace(imprintRegExp, function (wholeMatch, delim1, codeblock1, delim2, codeblock2) {
            ...
            var shortestIndention = code.replace(/(^|\n)\t/g, "$1    ")
                ...
                .reduce(function (minIndent, match) {
...
var cleanHideInOverviewComment = {
    type: 'listener',
    listeners: {
        'headers.before': function (event, text, options, globals) {
...
var cleanSectionDescriptionComment = {
    type: 'listener',
    listeners: {
        'headers.before': function (event, text, options, globals) {
...
var cleanEmptyParagraphs = {
    type: 'output',
    filter: function (text, converter) {
        var cleanBefore = ['div', 'iframe'];
        text = text.replace(/<p><\/p>/g, "");
        for (var _i = 0, cleanBefore_1 = cleanBefore; _i < cleanBefore_1.length; _i++) {
            var element = cleanBefore_1[_i];
...
var cleanMarkdownAttribute = {
    type: 'output',
    filter: function (text, converter) {
        // check for HTML elements containing a markdown attribute
        var markdownAttributeRegExp = /<(\S+)((?:[ \t]+(?!markdown[ \t]*=[ \t]*["'])\S+[ \t]*=[ \t]*(["'])(?:\\\3|(?!\3).)*\3)*)([ \t]+markdown[ \t]*=[ \t]*(["'])(?:\\\5|(?!\5).)*\5)([^>]*)>/gi;
        return text.replace(markdownAttributeRegExp, function (wholeMatch, tag, before, wrapBefore, attr, wrap, after, closingSlash) {
            // remove the attribute
            return wholeMatch.replace(attr, "");
        });
    }
};
var elearnHtmlBody = function () {
    return [
        addSectionOnHeading,
...
var elearnImprint = function () {
    return [parseImprint];
};
var parseSection = function (converter, wholeMatch, wrap, heading, addition) {
    ...
    if (addition.match(secDescriptionRegExp)) {
        ...
        addition.replace(secDescriptionRegExp, function (wholeMatch, content) {
...
var createMeta = function (tag, value, valueSurrounding) {
    // unescape
    if (valueSurrounding) {
...
*/
var removeIndention = function (block, indentionSize, tabSize) {
    if (indentionSize === 0)
        return block;
    ...
    return block.replace(spaceRegExp, function (wholeMatch, lineBreak, indention) {
        return lineBreak;
    });
};
...
//# sourceMappingURL=ShowdownElearnJS.js.map